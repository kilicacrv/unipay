...
import { Users, MapPin, BarChart2, LogOut, Bell, Zap, Activity } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 font-sans antialiased text-slate-900 pt-20 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-[10rem] z-20 py-4 px-4 overflow-x-auto no-scrollbar shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center gap-3">
          ...
          <Link 
            to="/admin/flash-campaigns"
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border ${location.pathname.includes('/admin/flash-campaigns') ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
          >
            <Zap size={14} /> Flaş Kampanyalar
          </Link>
          <Link 
          ...
          <Link 
            to="/admin/logs"
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border ${location.pathname.includes('/admin/logs') ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}
          >
            <Activity size={14} /> Sistem Logları
          </Link>
        </div>
      </div>
